...
class CrfTaggerTest(ModelTestCase):
    ...
    def test_simple_tagger_can_train_save_and_load_ccgbank(self):
        self.ensure_model_can_train_save_and_load(
            FIXTURES_ROOT / "ner" / "crf_tagger" / "experiment_ccgbank.json"
        )

    def test_simple_tagger_can_train_save_and_conll2000(self):
        self.ensure_model_can_train_save_and_load(
            FIXTURES_ROOT / "ner" / "crf_tagger" / "experiment_conll2000.json"
        )
    ...
    def test_low_loss_for_pretrained_transformers(self):
        self.set_up_model(
            FIXTURES_ROOT / "ner" / "crf_tagger" / "experiment_albert.json",
            FIXTURES_ROOT / "ner" / "conll2003.txt",
        )
        training_tensors = self.dataset.as_tensor_dict()
        output_dict = self.model(**training_tensors)
