...
# from allennlp_models.rc import models as rc_models
# from allennlp_models.structured_prediction import models as sp_models
# from allennlp_models.tagging import models as tagging_models
...
MODEL_CONFIG_MAP = {}
for i in PretrainedModelConfiguration.get_instances():
    MODEL_CONFIG_MAP[i.__dict__["id"]] = i.__dict__

MODEL_CONFIG_MAP["glove-sst"] = {
...
def get_predictor(model: str) -> Predictor:
    try:
        archive_file = MODEL_CONFIG_MAP[model]["archive_file"]
        predictor_name = MODEL_CONFIG_MAP[model].get("predictor_name")
        Predictor.from_path(archive_file, predictor_name)
    except KeyError:
