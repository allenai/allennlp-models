...
class Squad2Reader(DatasetReader):
    """
    Reads a JSON-formatted SQuAD 2.0 file and returns a ``Dataset`` where the ``Instances`` have four
    fields: ``question``, a ``TextField``, ``passage``, another ``TextField``, and ``span_start``
    and ``span_end``, both ``IndexFields`` into the ``passage`` ``TextField``.  We also add a
    ``MetadataField`` that stores the instance's ID, the original passage text, gold answer strings,
    and token offsets into the original passage, accessible as ``metadata['id']``,
    ``metadata['original_passage']``, ``metadata['answer_texts']`` and
    ``metadata['token_offsets']``. This is so that we can more easily use the official SQuAD 2.0
    evaluation script to get metrics.

    This differs from the SquadReader in that we append a special no-answer token to each
    context. If the model's argmax span is this token, then we predict that the question is unanswerable,
    ...
    def text_to_instance(
        ...
        if self.passage_length_limit is not None:
            passage_tokens = passage_tokens[: self.passage_length_limit]  # type: ignore

        passage_tokens_with_no_answer = passage_tokens + [  # type: ignore
            Token(
                text=SQUAD2_NO_ANSWER_TOKEN,
                idx=passage_tokens[-1].idx + len(passage_tokens[-1].text) + 1,  # type: ignore
                lemma_=SQUAD2_NO_ANSWER_TOKEN,
            )
        ]
        ...
        if is_impossible is True:
            answer_texts = [SQUAD2_NO_ANSWER_TOKEN]
            token_spans: List[Tuple[int, int]] = [
                (len(passage_tokens_with_no_answer) - 1, len(passage_tokens_with_no_answer) - 1)
            ]
        else:
            ...
            token_spans = []
            passage_offsets = [
                (token.idx, token.idx + len(token.text)) for token in passage_tokens_with_no_answer
            ]
            for char_span_start, char_span_end in char_spans:
                ...
                if error:
                    ...
                    logger.debug(
                        "Passage tokens (with no-answer): %s", passage_tokens_with_no_answer
                    )
                    ...
                    logger.debug(
                        "Tokens in answer: %s",
                        passage_tokens_with_no_answer[span_start : span_end + 1],
                    )
            ...
            if char_spans and not token_spans:
                if self.skip_invalid_examples:
                    ...
                    token_spans.append(
                        (
                            len(passage_tokens_with_no_answer) - 1,
                            len(passage_tokens_with_no_answer) - 1,
                        )
                    )
        return utils.make_reading_comprehension_instance(
            question_tokens,
            passage_tokens_with_no_answer,
